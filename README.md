...
*   [⚙️ How it Works: The Setup Process](#️-how-it-works-the-setup-process)
...
*   [🛠️ The Configuration Wizard: Your Control Panel](#️-the-configuration-wizard-your-control-panel)
*   [🔒 Essential Secrets: Manual Configuration Required!](#-essential-secrets-manual-configuration-required)
*   [☁️ Deploying Your dApp](#️-deploying-your-dapp)
...
*   **🖼️ NFT Gallery:** Dynamically browse minted NFTs from your configured smart contracts on various networks.
*   **✨ NFT Minting:** A dedicated interface for an authorized minter (set by the contract owner) to create new NFTs.
*   **🛡️ Owner Panel:** Secure section for the contract owner to manage critical settings like the authorized minter address and the base URI for metadata.
...
## ⚙️ How it Works: The Setup Process
...
### GitHub Codespaces (Recommended for Easiest Start)
...
## 🛠️ The Configuration Wizard: Your Control Panel
...
2.  **Network Configuration:**
    *   For several predefined blockchains (Ethereum, Sepolia, Polygon, etc.) and any custom chains you add:
        ...
        *   **RPC URL:** The connection link to the blockchain. Public ones are often okay for reading data. **If your RPC URL contains a private API key, it's more secure to set it in your `.env` file (local) or Codespaces/Vercel Secrets instead of here.** The app will use the `.env`/secret value if it exists, otherwise it will use what's entered here.
    ...
    *   You can also **Add Custom Network Chains** if you use a blockchain not on the default list.
...
3.  **Private RPC URLs (If you use them):**
    *   If your RPC URL from a provider (like Infura, Alchemy, QuickNode) contains a **private API key** (e.g., `https://mainnet.infura.io/v3/YOUR_PRIVATE_KEY`), you should set it as an environment variable rather than in the wizard if you plan to share `projectConfig.json` publicly.
    *   **Variable Name Format:** `VITE_<NETWORK_KEY>_RPC_URL` (e.g., `VITE_ETHEREUM_RPC_URL`, `VITE_SEPOLIA_RPC_URL`).
    *   The application will prioritize RPC URLs from these environment variables. If not set, it will use the (preferably public) RPC URL entered in the wizard (from `projectConfig.json`).

**Ensure these secrets are correctly set up in the appropriate environment before expecting all features to work!**

## ☁️ Deploying Your dApp
